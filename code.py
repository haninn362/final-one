...
# + Analyse de sensibilité — MINIMAL STREAMLIT WRAPPER
# ==================================================
...
SERVICE_LEVELS = [0.90, 0.92, 0.95, 0.98]   # Sensibilité

# --- STREAMLIT SETUP ---
st.set_page_config(page_title="PFE HANIN - Simulation Stock", layout="wide")
st.title("📊 PFE HANIN — Base Stock + Forecast + Simulation")
st.write("Cliquez sur le bouton ci-dessous pour lancer la simulation complète.")
...
if st.button("🚀 Lancer la Simulation"):
    ...
    def run_sensitivity(best_per_code, qr_map):
        ...
        for sl in SERVICE_LEVELS:
            ...
            if not df_run.empty:
                ...
                st.write(f"\n=== Résumé SL={sl:.2f} ===")
                st.dataframe(summary)  # ← Replaced display()
                all_results.append(df_run)
        return pd.concat(all_results, ignore_index=True)

    # ==================================================
    # MAIN EXECUTION (triggered by button)
    # ==================================================
    ...
    st.write("\n=== ✅ Meilleure méthode par article (critère: RMSE) ===")
    st.dataframe(best_per_code)  # ← Replaced display()
    ...
    # ✅ OPTIONAL: Save outputs to Excel (uncomment if needed)
    # final_results.to_excel("final_simulation.xlsx", index=False)
    # sensitivity_results.to_excel("sensitivity_analysis.xlsx", index=False)
    # best_per_code.to_excel("best_methods.xlsx", index=False)
